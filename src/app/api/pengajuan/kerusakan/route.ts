...
import { createLaporanKerusakanSchema } from "@/lib/validation/pengajuan";
import { createLaporanKerusakanEntry } from "@/server/services/pengajuan.service";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = createLaporanKerusakanSchema.safeParse(body);
    if (!parsed.success) {
      throw ApiError.unprocessable("Validasi gagal", parsed.error.flatten().fieldErrors as Record<string, string>);
    }

    const pengajuan = await createLaporanKerusakanEntry(parsed.data, fotoFile, session.sub);
